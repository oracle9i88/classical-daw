...
- 钢琴卷帘中点击空白区域新增音符；点击音符后可编辑音高、起始拍、时值和力度。卷帘支持 4、8、16 小节显示，音符拖动会按四分之一拍量化并限制在可见范围内。
- 选中音符后可用 ⌘/Ctrl+C、⌘/Ctrl+V 复制粘贴，方向键以四分之一拍或半音移动。
- 使用浏览器原生 Web Audio API 播放三角波试听，速度可调（30–240 BPM）。
- 选中音符后支持歌词、撤销/重做，以及 2/4、3/4、4/4、6/8 拍号编辑。
- 导入网页导出的 MusicXML（也支持相同单声部/多 voice 子集的其他文件），解析音符、休止、和弦、跨小节连音、单个歌词文本、力度和速度后更新卷帘。
...
- 导出 48 kHz、PCM16 单声道 WAV，使用与网页试听一致的确定性诊断振荡器。
- 可保存/打开版本化 JSON 工程文件；旧版工程缺少拍号或小节数时按 4/4、4 小节兼容，坏文件不会覆盖当前工程。
...
这是可运行的交互原型，不是完整 DAW：网页仍是单页单乐器时间线，网格显示 4、8 或 16 小节、C4–B5，拍号支持常用的 2/4、3/4、4/4、6/8；尚未包含谱面排版、力度曲线、真实采样器或 CoreAudio 设备控制。MusicXML 导入/导出限定为这一可视范围内的单声部、多 voice 和弦子集：960 divisions、音符/休止、跨小节连音、单个歌词文本、速度、力度和首个拍号。MIDI 导入支持标准 Type 0/1 的 PPQ 文件，严格校验轨道、VLQ、EOT、音符配对、tempo 与拍号，并按文件长度自动选择 4/8/16 小节；超过 16 小节或 C4–B5 的内容会被裁剪/忽略，非四分之一拍会量化。MIDI 导出为 Type 1、960 PPQ，生成 conductor tempo/拍号轨和音符轨。网页 WAV 导出是 48 kHz PCM16 单声道诊断音色，不是生产采样器。网页 JSON 工程文件只保存当前原型的音符编辑状态，不是完整项目包；`measures` 字段可选，缺失时兼容为 4 小节；浏览器本地副本也只用于恢复当前原型状态。
