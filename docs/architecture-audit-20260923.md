...
2026-09-23 后续实时门也已通过：同一 AU/输出生命周期内连续三次换页，
下一个 engine quantum 接收新修订，跨小节延音只触发/释放各一次，
踏板与表情追赶、采集无静音缺口。十音和弦当时测的是子块代理指标：
最坏占比从 72.4% 变为 48.84%，两次均未记录该子块指标的 100% 越界，
但没有测完整客户端回调总时长，也未观测 OS xrun。均衡拆块保留；两次
最大子块耗时相差约 9%，不能据此证明因果加速或整体负载下降。
本次余量门改为完整 DefaultOutput 输入回调耗时除以其客户端帧时长，
不把回调外的采样率转换/驱动时间冒称已测。产品同时明确拒绝本轮已处理
起音的音符的起点修改：包括已起音后释放、以及因同键冲突而跳过的触发，
后者不代表真正发过声。锁定持续到本轮结束，不因改音高而清除。
音频边界裁决、确认后才提交历史，拒绝
整条命令并给可见原因，停止播放后可修改。见
[完整语义、失败记录和验收](live-performance.md)。旧 ScoreHistory 已去除
重复复制全部保留快照的开销，尚未等同于旧工具的增量命令迁移。
