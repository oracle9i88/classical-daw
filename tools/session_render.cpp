...
int main(int argc, char** argv) {
  try {
    ...
    const fs::path input = check_only ? argv[2] : argv[1];
    ...
    report << std::setprecision(17) << "{\n  \"format\": \"classical-daw-session-bounce-1\","
           << "\n  \"sample_rate\": 48000,\n  \"channels\": 2,\n  \"frames\": " << plan.frames
    ...
    for (std::size_t i = 0; i < plan.tracks.size(); ++i) {
      ...
      auto stem = instrument.render(routed.midi, &diagnostics, 48000, 5, false, plan.end_tick);
      ...
      daw::applyTrackMix(stem, routed.route.gain_db, routed.route.balance);
      ...
      report << "    {\"part_id\": " << quote(routed.route.part_id)
             ...
             << ", \"plugin_over_unity_samples\": " << diagnostics.over_unity_samples << '}';
    }
    ...
    report << "\n  ],\n  \"master_peak\": " << master.peak << ",\n  \"master_rms\": " << master.rms
           << ",\n  \"clipped_samples\": 0,\n  \"stem_tap\": \"post-track-fader, pre-master\"\n}\n";
    bundle.path("score.dawproj.tmp");
    if (!daw::writeProjectFile(score, bundle.path(saved.score_file).string(), &error) ||
        !daw::writeWavPcm16(mix, bundle.path("mix.wav").string(), &error)) throw std::runtime_error(error);
