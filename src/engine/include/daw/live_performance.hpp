...
class LivePerformanceStream {
 public:
  ...
  // Last fallible control operation before accepting an editor command. Failure
  // publishes nothing. A full two-slot mailbox rejects rather than overwriting.
  void submit(MidiSampleSequence sequence, double gain, std::uint64_t revision);
  ...
  bool hasPendingUpdate() const noexcept { return appliedRevision() != requested_.load(std::memory_order_acquire); }
 ...
 private:
  struct Plan;
  struct Voice { bool fired = false, down = false; std::uint8_t channel = 0, pitch = 0; };
